...
public class Ocean {
	...
	static final int NB_POISSONS_PILOTES_MAX = 10;
	...
	Ocean() {
		...
		while (compteur<CASES_POISSONS_PILOTES) {
			int ligne = r.nextInt(TAILLE_OCEAN);
			int colonne = r.nextInt(TAILLE_OCEAN);
			int random = r.nextInt(NB_POISSONS_PILOTES_MAX);
			if(zones[ligne][colonne].getNbPoissonsPilotes()+ random< NB_POISSONS_PILOTES_MAX) {
